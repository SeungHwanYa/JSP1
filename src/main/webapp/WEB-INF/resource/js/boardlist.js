...
// fetch -> 함수 fetch(url)
// fetch는 url과 options을 넣으면 해당하는 url로 request를 보내고 response를 받는 일방형 통신 기술
...
ajax("/api/boardlist", {
    ...
    var template = boards.querySelector("#boards template");
    for (var _i = 0, _a = json.data; _i < _a.length; _i++) {
        ...
        template.content.querySelector(".author").innerHTML = data.user.name;
