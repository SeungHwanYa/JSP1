...
<%--
	...
	<% request.getHeaders("Content-Dispostion"); %>;별로 분리
	...
	<% request.getCharacterEncoding(); %> 일반적으로 자동으로 UTF-8
	<% request.getServletPath();%>
	<% request.getServletContext();%>
	<% request.getPathInfo(); %> /A/B/C.jsp
	<% request.getParts();%>
	<% request.getPart("file");%> multipart/from-data
	...
	<% response.sendRedirect("/text.html");%> 해당 페이지로 이동(경로 자체를 변경) --%>
	<%-- <% response.sendError(404, "파일을 못찾았어!"); %>
	...
	<% session.invalidate(); %> 세션의 만료 기한을 현재로 변경(session을 없애는 기능)
 --%>
